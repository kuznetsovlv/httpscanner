	...
	class Job extends Responser {
		constructor (response, name, server) {
			...
			this.clear = (code, msg) => {
				...
				delete this.server.jobs[this.name];
				this.sendString(msg || '', code || 200, 'Conection closed');
	...
	class Server extends http.Server {
		constructor (root, port) {
			...
			Object.defineProperties(this.jobs, {
				...
				once: {
					value: (type, handler) => {
						let once = this.jobs.events.once;
						if (once[type])
			...
			this.on('initJob', (response) => {
				...
				this.jobs[name] = new Job(response, name, this);
			...
			this.on('post', (request, response) => {
				...
				if (!url) {
					...
					request.on('end', () => {
							if (!this.jobs[url])
								new Job (response, '').sendError(400, 'Incorrect url: ' + url);
							else
								try {
									this.jobs[url].setResponse(response).emit('data', request.headers['content-type'], data.join(''));
								} catch (e) {
									console.log(e);
									new Job (response, '').sendError(520, 'Unknown Error: ' + e);
								}
					});
				}
			});
